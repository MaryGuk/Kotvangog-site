...
export const NextArrow = ({ onClick }) => {
  return (
    <StyledIconButtonNext onClick={onClick}>
      <ArrowForwardIosIcon />
    </StyledIconButtonNext>
  );
};
...
export const PrevArrow = ({ onClick }) => {
  return (
    <StyledIconButtonPrev onClick={onClick}>
      <ArrowBackIosIcon />
    </StyledIconButtonPrev>
  );
};
...
const MobileCarouselItem = ({ onClick, onImageLoad, data }) => {
  ...
  return redirectTo ? <Link to={redirectTo}>{itemContent}</Link> : itemContent;
};
            ...
            >
              {images.map((imageData, idx) => (
                <MobileCarouselItem
                  key={idx}
                  data={imageData}
                  onClick={() => onImageClick(index * imagesOnPage + idx)}
  ...
  return (
    <Box>
      {carouselRowsImages.map((rowImages, idx) => (
        <Box mt={mt ?? '22px'} key={idx}>
          <MobileCarouselRow
            columnCount={columnCount}
            imageList={rowImages}
            onImageClick={(rowImagesIdx) =>
              onImageClick(
                imagesOnPage * Math.floor(rowImagesIdx / columnCount) +
                  (rowImagesIdx % columnCount) +
                  idx * columnCount
              )
            }
          />
